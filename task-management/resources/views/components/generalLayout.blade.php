    ...
    <div class="container-fluid">
      ...
      <div class="collapse navbar-collapse" id="navbarSupportedContent">
        <ul class="navbar-nav me-auto mb-2 mb-lg-0">
          ...
          <li class="nav-item">
            <a class="nav-link text-white text-xl cursor-pointer " href="{{route('myAssignedTask',['userId'=>$user->id])}}"><i class="fas fa-tasks p-2 text-xl cursor-pointer"></i>My Task</a>
          </li>
        </ul>
        <a class="nav-link text-white text-xl cursor-pointer" onclick="showRegistrationForm('{{$user->name}}')" ><i class="fas fa-user-plus p-2 text-xl cursor-pointer" ></i>Register Employees</a>

        <form action="{{ route('retrieveUsers',['id'=>$user->id]) }}" class="d-flex">
...
<!---offcanvas--->
  <div class="offcanvas offcanvas-start bg-blue-600" tabindex="-1" id="offcanvasExample" aria-labelledby="offcanvasExampleLabel">
    ...
    <div class="flex flex-row">
        <div>
          <button onclick="showEdditForm('{{$user->email}}','{{$user->id}}')" class="text-white border border-lg rounded-xl p-2 hover:bg-blue-500">Edit Profile</button>
        </div>
        <div>
          <button class="text-white border ml-6 px-12 border-lg border-red-500 rounded-xl hover:bg-red-500"> <a href="{{route('logout')}}" class="nav-link text-white text-xl p-2 cursor-pointer" ><i class="fas fa-sign-out-alt text-white text-xl cursor-pointer"></i> Logout</a></button>
        </div>
    </div>
